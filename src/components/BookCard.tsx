...
export const BookCard = ({ id, title, description, price, image, rating = 5, backgroundClassName, innerBackgroundClassName }: BookCardProps) => {
  return (
    <Link to={`/book/${id}`} target="_blank" rel="noopener noreferrer" className="block">
      <Card className="group hover:shadow-[var(--shadow-warm)] transition-all duration-300 hover:-translate-y-2 overflow-hidden border-2 animate-scale-in relative cursor-pointer">
